...
14. **O bucket `generated-recursos` não está versionado.** É criado à mão no Dashboard, então toda stack recriada do zero falha no upload do PDF com `Bucket not found` — aconteceu em 01/09/2026. **Demonstrado literalmente em 06/09/2026:** depois de um `supabase db reset`, o bucket `evidencias` (criado pela migration da Fase 1) estava lá e o `generated-recursos` tinha sumido. A migration `20260906000000_radar_inmetro_rj.sql` já traz o `insert into storage.buckets … on conflict do nothing` que resolve isto — **falta só replicar o padrão para o `generated-recursos`**, o que é trabalho de minutos e deve sair numa branch própria, não na do radar.
...
23. **O schema remoto precisa ser RECONSTRUÍDO, não reparado.** A baseline de 09/09/2026 não é um retrato: corrige o que as doze migrations deixaram errado. Marcar as antigas como `reverted` com `migration repair` faria as correções nunca rodarem em produção — elas nasceriam registradas como aplicadas. É viável porque as tabelas do fluxo em produção estão **vazias** (conferido no dia), mas é operação destrutiva e ainda não foi feita. Enquanto não for, produção segue com o schema antigo e sem bucket nenhum.
