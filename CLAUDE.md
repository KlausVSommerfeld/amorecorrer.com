...
Os testes Python do pipeline rodam fora do venv, de dentro de `pipeline/`: `python3 -m unittest test_verificacao test_prompt test_peca`. **Nunca `unittest discover`** — `test_resend_smtp.py` casa com o padrão e manda um e-mail real ao ser importado. O `npm run radar:test` cobre também `supabase/functions/form-submit/*.test.ts`.
...
Cinco invariantes que quebram em silêncio se forem desfeitas:
...
- **A verificação do medidor nunca derruba o envio, e nunca vai crua ao prompt.** A `form-submit` chama `verificar_medidor` com timeout de 3 s antes do `attempt_dispatch`, e grava o resultado em `form_submissions.verificacao_medidor` e em `radar_consultas_log`; qualquer falha vira `status: "nao_aplicavel"` e o envio segue. No pipeline, `verificacao_medidor` está sempre em `CAMPOS_INTERNOS` (`pipeline/prompt.py`) e só entra como bloco em português, com `RADAR_TESE_ATIVA=true`. A regra de redação é escolhida em código (`pipeline/verificacao.py`), não pelo modelo — e **vigência comprovada é silêncio** (sem bloco e sem as regras do radar no system prompt): com um bloco dizendo "não mencione", o DeepSeek mencionou em duas rodadas seguidas. Atenção ao efeito colateral: as regras do radar (só CTB art. 280, nada de resolução do CONTRAN) entram **só quando há bloco** — com a chave ligada, um caso `nao_aplicavel` ou comprovado vai ao modelo sem trava nenhuma sobre o tema, como antes da feature. E `nao_comprovado` com registro cobrindo a data (Pendente, Reparado) usa uma instrução própria: a peça diz que a base registra verificação **sem aprovação**, nunca que "não localizou".
- **A forma da peça é garantida por código, não pelo modelo.** O prompt manda texto puro e parar no "pede deferimento", mas `pipeline/peca.py` limpa o markdown que escapar, corta o que vier depois do pedido e monta o fecho a partir do caso — cidade do cliente, **data sempre em branco** (é o dia do protocolo, que o sistema não sabe), nome e CPF. Dado ausente vira linha em branco (`________`), nunca `[marcador]`. E o PDF troca `\n` por `<br/>`: sem isso o reportlab junta linhas.
