...
**`src/components/ui/` tem um arquivo só: `accordion.tsx`.** Os outros 48 componentes que o scaffold do Lovable trouxe foram apagados em 20/09/2026 — nenhum era alcançável a partir de `src/main.tsx`, e com eles saíram 41 dependências de produção sem um único import. Não presuma que `@/components/ui/<qualquer coisa>` existe: o `components.json` continua versionado, então o caminho é `npx shadcn@latest add <componente>`, que regenera o arquivo e instala a dependência. O Tailwind varre `content` em vez de fazer tree-shaking, então cada componente reintroduzido volta a pesar no CSS — a remoção dos 48 encolheu a folha de 95,72 kB para 49,49 kB.
